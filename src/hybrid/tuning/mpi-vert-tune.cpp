...
void needlemanWunsch(dnaArray s1, dnaArray s2, long int nCols,
                     int rank, int nProc, int* t, int bufSize) {
  ...
  if (rank == 0) {
    ...
    for (long int h = 1; h < nRows; h += bufSize) {
      long int iMax = (h + bufSize < nRows) ? h + bufSize : nRows;
      ...
      MPI_Send(sendBuf, bufSize, MPI_INT, 1, 0, MPI_COMM_WORLD);
    }
  }
  ...
  // never sends; only receives from rank-1
    ...
    for (long int h = 1; h < nRows; h += bufSize) {
      ...
      MPI_Recv(recvBuf, bufSize, MPI_INT, rank-1, rank-1,
               MPI_COMM_WORLD, NULL);
  ...
  // receive at the start, then send
    ...
    for (long int h = 1; h < nRows; h += bufSize) {
      ...
      MPI_Send(sendBuf, bufSize, MPI_INT, rank+1, rank, MPI_COMM_WORLD);
    }
  }
...
#include "mpi-vert-tune-driver.cpp"
