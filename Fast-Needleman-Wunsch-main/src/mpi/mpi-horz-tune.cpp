...
void needlemanWunsch(dnaArray s1, dnaArray s2, long int nRows,
  ...
  if (rank == 0) {
    ...
    for (long int h = 1; h < nCols; h += bufSize) {
      long int jMax = (h + bufSize < nCols) ? h + bufSize : nCols;
  ...
  // receive at the start, then send
    ...
    for (long int h = 1; h < nCols; h += bufSize) {
      ...
      // receive into first row
      MPI_Recv(&t[h], bufSize, MPI_INT, rank-1, rank-1,
               MPI_COMM_WORLD, NULL);
...
#include "mpi-horz-tune-driver.cpp"
